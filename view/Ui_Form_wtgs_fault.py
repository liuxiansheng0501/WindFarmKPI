...
class Ui_fault_info(object):

    def setupUi(self, fault_info):

        fault_info.setObjectName("fault_info")
        fault_info.resize(800, 600)
        self.VLayoutWidget = QtWidgets.QWidget(fault_info)
        ...
        self.retranslateUi(fault_info)
        QtCore.QMetaObject.connectSlotsByName(fault_info)

    def retranslateUi(self, fault_info):

        _translate = QtCore.QCoreApplication.translate
        fault_info.setWindowTitle(_translate("fault_info", "机组故障信息"))

if __name__ == "__main__":
    import sys
    app = QtWidgets.QApplication(sys.argv)
    fault_info = QtWidgets.QDialog()
    ui = Ui_fault_info()
    ui.setupUi(fault_info)
    fault_info.show()
    sys.exit(app.exec_())
